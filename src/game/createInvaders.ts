...
import {blocks} from './invaderLayout.ts'

export function createInvaders(invaderSize: number, multiplier: number, speedX: number): Invader[] {
  ...
  for (let y = 0; y < rows; y++) {
    ...
    for (const xIndex of rowPixels) {
      invaders.push(
        new Invader(xIndex * invaderSize, y * invaderSize, speedX)
      )
    }
  }
